...
- A monitored URL can now be paused and resumed (`PATCH`-style server action, `updateMonitoredUrlStatusAction` → `updateMonitoredUrlStatus`) instead of only ever being added, closing a gap where the per-site cap (`SCCC_MAX_MONITORED_URLS_PER_SITE`, default 10) had no way to be freed up once reached. Pausing sets `isActive: false` without deleting the row, so its snapshot/event/regression history stays intact on the timeline; the scheduled rescan job already only considered `isActive: true` rows, so a paused URL now genuinely stops being scanned. Resuming re-checks the same cap used at creation time so a paused URL cannot be used to sneak past the limit. The Monitored URLs table shows a Status column (Active/Paused) and swaps the row's action between Rescan+Pause and Resume accordingly.
