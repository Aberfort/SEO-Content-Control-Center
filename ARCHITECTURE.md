...
- Production deployment descriptors now exist for a portable first-server path: one shared multi-target `Dockerfile`, `docker-compose.production.example.yml` for SaaS/marketing/worker/migrations plus local Postgres/Redis, `.env.production.example`, and `npm run deploy:smoke`. A platform-specific deployment can still replace this with managed services or provider-native configuration.
