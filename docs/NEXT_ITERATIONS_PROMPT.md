...
Узгоджені продуктові, маркетингові, deployment-packaging, plugin-release, dependency-audit, Vercel production-build, локальна WordPress plugin connection UX і WordPress sync datetime compatibility ітерації виконані. Репозиторний кодовий launch gate закритий: format/lint/test/build, Prisma migration deploy і `npm audit --audit-level=low` мають проходити на актуальному дереві залежностей.
