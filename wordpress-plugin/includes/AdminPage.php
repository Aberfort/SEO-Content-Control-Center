...
final class AdminPage {
	...
	private function renderSummary( array $summary, int $completedAt ): void {
		$cards = array(
			array(
				'label' => __( 'Audited URLs', 'seo-content-control-center' ),
				'value' => (int) ( $summary['total_urls'] ?? 0 ),
				'tone'  => 'neutral',
			),
			array(
				'label' => __( 'Need review', 'seo-content-control-center' ),
				'value' => (int) ( $summary['affected_urls'] ?? 0 ),
				'tone'  => 'attention',
			),
			array(
				'label' => __( 'Critical', 'seo-content-control-center' ),
				'value' => (int) ( $summary['critical'] ?? 0 ),
				'tone'  => 'critical',
			),
			array(
				'label' => __( 'Checks complete', 'seo-content-control-center' ),
	...
	private function renderFindings( array $items, ?array $connection ): void {
		...
		<form class="sccc-filters" method="get" action="<?php echo esc_url( admin_url( 'admin.php' ) ); ?>">
			...
			$this->renderFilterSelect(
				'sccc_type',
				$postType,
				__( 'All content types', 'seo-content-control-center' ),
				array(
					'post' => __( 'Posts', 'seo-content-control-center' ),
					'page' => __( 'Pages', 'seo-content-control-center' ),
				)
			);
			?>
			<?php
			$this->renderFilterSelect(
				'sccc_change',
				$change,
				__( 'Any change', 'seo-content-control-center' ),
				array(
					'new'       => __( 'New since last audit', 'seo-content-control-center' ),
					'unchanged' => __( 'Unchanged', 'seo-content-control-center' ),
				)
			);
			?>
			<?php
			$this->renderFilterSelect(
				'sccc_visibility',
				$visibility,
				__( 'Active findings', 'seo-content-control-center' ),
				array(
					'all'     => __( 'Active and ignored', 'seo-content-control-center' ),
					'ignored' => __( 'Ignored only', 'seo-content-control-center' ),
				)
			);
			?>
