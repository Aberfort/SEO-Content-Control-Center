...
final class SafeOperationEndpoint {
	...
	public function registerRoutes(): void {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => array( $this, 'handleApply' ),
	...
	private function normalizeAfterValue( array $afterValue ): array {
		$supportedFields = array_merge( array( 'seoPlugin', 'canonicalUrl' ), array_keys( self::TEXT_FIELDS ), self::BOOL_FIELDS );
		$unknownFields   = array_values( array_diff( array_keys( $afterValue ), $supportedFields ) );

		if ( array() !== $unknownFields ) {
			return array( 'error' => 'unsupported_field:' . implode( ',', $unknownFields ) );
		}

		$seoPlugin = null;

		if ( array_key_exists( 'seoPlugin', $afterValue ) ) {
			if ( ! is_string( $afterValue['seoPlugin'] ) || ! in_array( $afterValue['seoPlugin'], array( 'yoast', 'rank_math' ), true ) ) {
				return array( 'error' => 'unsupported_seo_plugin' );
			}
		...
		foreach ( self::TEXT_FIELDS as $field => $maxLength ) {
			...
			if ( isset( $normalized['error'] ) ) {
				return array( 'error' => $normalized['error'] );
			}
		...
		if ( array_key_exists( 'canonicalUrl', $afterValue ) ) {
			...
			if ( isset( $normalizedUrl['error'] ) ) {
				return array( 'error' => $normalizedUrl['error'] );
			}
		...
		foreach ( self::BOOL_FIELDS as $field ) {
			...
			if ( null !== $afterValue[ $field ] && ! is_bool( $afterValue[ $field ] ) ) {
				return array( 'error' => 'invalid_' . $field );
			}
		...
		if ( array() === $values ) {
			return array( 'error' => 'no_supported_fields' );
		}
	...
	private function hasStoredMetaValue( int $postId, string $key ): bool {
		...
		if ( is_array( $value ) ) {
			return array() !== array_filter( $value, static fn ( mixed $item ): bool => is_scalar( $item ) && '' !== trim( (string) $item ) );
		}
	...
	private function normalizeRobotTokens( mixed $value ): array {
		if ( is_array( $value ) ) {
			...
			$rawTokens = preg_split( '/[\s,]+/', (string) $value ) ?: array();
		} else {
			$rawTokens = array();
		}
	...
	private function normalizeNullableText( mixed $value, string $field, int $maxLength ): array {
		...
		if ( $this->stringLength( $text ) > $maxLength ) {
			return array( 'error' => $field . '_too_long' );
		}
