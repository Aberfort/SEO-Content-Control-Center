...
final class LocalAuditRunner
{
    ...
    public function ensureRecurring(): string
    {
        ...
        if (function_exists('as_next_scheduled_action') && function_exists('as_schedule_recurring_action')) {
            if (false === as_next_scheduled_action(self::RECURRING_HOOK, [], self::GROUP)) {
                as_schedule_recurring_action(time() + $seconds, $seconds, self::RECURRING_HOOK, [], self::GROUP);
            }
    ...
    public function getRecurringStatus(): array
    {
        ...
        if ('off' === $interval) {
            return ['enabled' => false, 'interval' => 'off', 'scheduler' => null, 'next_run_at' => null];
        }
        ...
        if (function_exists('as_next_scheduled_action')) {
            ...
            if (is_int($next) && $next > 0) {
                return ['enabled' => true, 'interval' => $interval, 'scheduler' => 'Action Scheduler', 'next_run_at' => $next];
            }
        }
