...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const operation = await repository.rollbackBulkOperation({
      user,
      organizationId,
      siteId,
      operationId,
    ...
    if (error instanceof Error && error.message === "BULK_OPERATION_NOT_READY") {
      return jsonError(
        ...
        "Bulk operation is not completed or failed and cannot be rolled back."
      );
    }

    if (error instanceof Error && error.message.startsWith("Role ")) {
      return jsonError(403, "FORBIDDEN", "Your role does not allow rolling back bulk operations.");
    }
