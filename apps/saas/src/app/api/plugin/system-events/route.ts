...
import { acceptPluginSystemEventBatch, authenticatePluginSyncRequest } from "@/lib/plugin-connection";
...
export async function POST(request: Request) {
  ...
  try {
    await assertRateLimit("plugin-system-events", rateLimitKeyFromHeaders(request.headers));
    const authentication = await authenticatePluginSyncRequest({
      request,
      bodyText
    });
    const data = await acceptPluginSystemEventBatch({
      authentication,
    ...
    return jsonError(400, "PLUGIN_SYSTEM_EVENT_FAILED", "Could not accept plugin system events.");
  }
}
